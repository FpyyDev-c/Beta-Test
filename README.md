...
## 🆕 Servidores e painel de admin (FpyyCord)

Além das salas de compartilhamento de tela, o projeto agora tem um sistema de
**servidores permanentes**, parecido com o Discord:

- `servers.html` — lista de servidores públicos, criar servidor (público ou
  privado, com código de entrada, limite de membros, ícone e banner),
  entrar por código, e editar seu perfil (nome, foto, banner, descrição).
- `admin.html` — painel de admin protegido por senha: gerencia todos os
  servidores (tornar público/privado, excluir) e todos os perfis (remover).
- `index.html` agora só redireciona para `servers.html`.

**Isso precisa de configuração antes de funcionar**, porque o site não tem
backend próprio: os dados (lista de servidores, perfis) são guardados no
próprio repositório do GitHub, lidos e gravados via API. Antes de publicar:

1. Abra `js/config.js` e preencha o bloco `GITHUB_DB`:
   - `OWNER`: seu usuário do GitHub.
   - `REPO`: o nome deste repositório.
   - `TOKEN`: um **Fine-grained personal access token** (crie em
     https://github.com/settings/personal-access-tokens/new), com acesso
     **somente a este repositório** e permissão **somente de "Contents:
     Read and write"**.
2. Troque `ADMIN_PASSWORD` (mesmo arquivo) por uma senha sua para acessar
   `admin.html`.

⚠️ **Esse token fica visível no código-fonte público do site** — qualquer
pessoa pode abrir o `js/config.js` publicado e lê-lo. Por isso é essencial
usar um token com acesso limitado só a este repositório (nunca um token
"classic" com acesso amplo à sua conta). Veja mais detalhes nos comentários
de `js/gh-db.js`. Se preferir uma solução mais segura no futuro, o ideal
seria migrar para um banco de dados de verdade (ex: Supabase), que evita
esse risco.
