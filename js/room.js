...
(function () {
  ...
  if (!SR.isValidRoomCode(code)) {
    location.href = `servers.html?error=invalid-code`;
    return;
  }

  // Se veio de um servidor (servers.html), busca nome/ícone/limite de
  // membros e aplica antes de conectar. Se a busca falhar (ex: token do
  // GitHub não configurado ainda), a sala continua funcionando normalmente
  // com o limite padrão — isso nunca bloqueia quem entra por link direto.
  const serverId = params.get("server");
  let serverInfoPromise = Promise.resolve(null);
  if (serverId && window.SR.getServer) {
    serverInfoPromise = window.SR.getServer(serverId)
      .then((server) => {
        if (!server) return null;
        if (server.memberLimit) cfg.MAX_PARTICIPANTS = server.memberLimit;
        const chipAvatar = document.getElementById("serverAvatarChip");
        const chipName = document.getElementById("serverNameChip");
        if (chipName) {
          chipName.textContent = server.name;
          chipName.style.display = "inline";
          document.title = `${server.name} — FpyyCord`;
        }
        if (chipAvatar) {
          if (server.avatar) {
            chipAvatar.style.backgroundImage = `url('${server.avatar}')`;
          } else {
            chipAvatar.textContent = SR.initials(server.name);
          }
  ...
  serverInfoPromise.then(() => ensureNameThenBoot());
})();
